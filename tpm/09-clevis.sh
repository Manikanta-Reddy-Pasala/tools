...
status)
  ...
  clevis luks pass --help >/dev/null 2>&1 \
    || die "this clevis is too old for 'clevis luks pass' (needs v18+). Fall back to: add a known passphrase with 'cryptsetup luksAddKey $DEV' before clearing."
  while read -r slot pin cfg; do
    [[ "${pin:-}" == tpm2 ]] || continue
    echo
    info "slot $slot:"
...
unbind)
  ...
  confirm_typed "UNBIND CLEVIS FROM $DEV"
  for row in "${ROWS[@]}"; do
    set -- $row
...
MSG
  ...
  printf '%s' "$PASS" | clevis luks bind -d "$DEV" tpm2 \
